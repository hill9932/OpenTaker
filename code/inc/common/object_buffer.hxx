...
**/
#ifndef __VP_OBJEC_BUFFER_INCLUDE_H__
#define __VP_OBJEC_BUFFER_INCLUDE_H__
...
namespace LiangZhu
{
    ...
    {
    public:
        CObjectPool(u_int32 _count = 0)
        {
            m_objBuf = NULL;
            init(_count);
        }

        void reset(u_int32 _count)
        {
            init(_count);
        }
    ...
    private:
        void init(u_int32 _count)
        {
            if (m_objBuf)   delete[] m_objBuf;
    ...
    **/
    ...
    {
    public:
        CObjectPool3(u_int32 _objCount = 0) : CObjectPool<T>(_objCount)
        {
        }
        ...
        {
        retry:
            u_int32 curIndex = this->m_curIndex % this->m_objCount;
            u_int32 nextIndex = (curIndex + 1) % this->m_objCount;
            typename CObjectPool<T>::InteralObject* obj = &this->m_objBuf[curIndex];

            while (obj->used != 0)
            {
                obj = &this->m_objBuf[nextIndex++ % this->m_objCount];
                if (nextIndex >= this->m_objCount)
                {
                    nextIndex = 0;
                }
            }

            nextIndex %= this->m_objCount;
